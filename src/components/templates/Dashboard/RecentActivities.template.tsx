...
const RecentActivitiesTemplate: React.FC<RecentActivitiesProps> = ({
  ...
  return (
    <div>
      {activities.length === 0 ? (
        <div className="py-10 text-center rounded-xl border" style={{ borderColor: colors.neutral300, color: colors.neutral500 }} >
          🚀 No recent activities yet.
        </div>
      ) : (
        <div className="relative pl-10 pt-4 space-y-8 max-h-[450px] overflow-y-auto pr-2" style={{ borderColor: colors.neutral300 }}>
          <div className="absolute left-15 top-1 bottom-1 w-[2px] rounded-full mt-8" style={{ background: `linear-gradient(to bottom, ${colors.primary200}, ${colors.primary400})` }} />
          {activities.map((activity, index) => {
            ...
            return (
              <div key={index} className="relative flex items-center gap-4 group">
                <div className={`relative z-10 w-10 h-10 rounded-full flex items-center justify-center text-white shadow-sm transition-all duration-300 group-hover:scale-110 ${color} ${ring}`} style={{ boxShadow: `0 0 0 4px ${colors.neutral50}` }}>
                  <div className="text-xl">{icon}</div>
                </div>
                <div className={`flex-1 rounded-2xl p-4 border ${border}`} style={{ background: "#ffffff" }}>
